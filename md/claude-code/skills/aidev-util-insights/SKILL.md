...
3. **縦断分析＝過去に入れた改善の効果検証**（新しい提案より先に回す）。
   ...
   **(a) PJ規約の条項**（`protocol.md`「12.」／詳細は `protocol-conventions.md`）:
   ...
   - 判定は **`baseline`（起票時に刻んだ「前」）と、導入後のタグ件数の比較**で行う。
     **id 別の前後比較はできない**——条項 id は `introduced` の瞬間に生まれるので、
     導入前の review.md にその id は現れず、件数は `0 → N` と増えるだけになる。
   - **母集団は `introduced` 以降に着手し deliver 済みの works だけ**（`pop` と一致させる）。
     下の grep は全 works を舐めるので、**`pop` に合わせて対象を絞ってから数えること**。
   - `baseline` に「前を作れない」と書かれた条項は**比較が成立しない**。件数ではなく
     個別の根拠を示して判定し、示せないなら `pending` のまま置く（無理に確定させない）。
