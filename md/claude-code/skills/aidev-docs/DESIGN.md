...
- **`/goal`（Claude Code）は工程単位の停止前チェックとして使う**：このハーネスで実際に起きた事故は
  「**やるべきことを終える前に停止する**」型が多い（`guard` と `event start` が別コマンドであるため、
  review の start が両ラウンドとも欠落した実例がある）。`aidev verify` はこれを deliver 前に WARN で
  **事後検知**できるが、`/goal` は**停止の手前で防げる**。両者を**予防（`/goal`）と検知（`verify`）の
  二段**として使う。設定するのは「工程の完了の目安＋終了プロトコルの記録まで終えること」で、
  **`requirement.md` の `目的 / ゴール` とは別物**（あちらは永続する成果物、`/goal` はセッション内の
  揮発的ガード）。work のゴールを `/goal` に入れても停止条件として粗すぎて機能しないため、
  工程単位で設定し、工程を移るたびに更新する。`protocol.md`「3.2」。
