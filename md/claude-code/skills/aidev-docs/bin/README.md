...
| `convention status [--format table\|tsv] [--members <id>]` | **読み取り専用**。`--members <id>` は母集団の work 一覧（着手・deliver・`[conv:<id>]` 件数。subtask の review.md は親に合算）。条項の `status` / `introduced` / **`pop`（母集団＝導入日以降に着手し **deliver 済み**の work 件数）** / `need` / **`ready`（判定可能か）** / **`index`（索引に載っているか）** / `promoted_to` を一覧。`aidev-util-insights` の縦断分析の入口。 |
