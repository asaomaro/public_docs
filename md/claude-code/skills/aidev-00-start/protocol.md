...
| `/goal`（「3.2」） | 停止前チェック | 同じ内容を指示で書く | 自動性のみ |
...
### 3.2 `/goal` による停止前チェック（推奨）

Claude Code の `/goal`（*Set a goal Claude checks before stopping*）を**工程開始時に設定する**。
このハーネスで実際に起きた事故は「**やるべきことを終える前に停止する**」型が多く、`/goal` はその手前で止める。

- **設定するもの**: その工程の「完了の目安」＋**終了プロトコル（「3.」）の記録まで完了すること**。
  - 例（coding）: 「tasks.md の対象タスクを完了し、`aidev approve coding tasks_done=… unplanned_lookups=…`
    まで記録し終えている」
  - 例（review）: 「`review.md` をラウンドとして追記し、`aidev approve review must=… should=… nit=…`
    まで記録し終えている」
- **なぜ効くか**: `guard` と `event start` が別コマンドである以上、記録漏れは構造的に起こりうる
  （実例: review の start が両ラウンドとも欠落し、所要時間が導出できなくなった）。
  `aidev verify` はこれを **deliver 前に WARN で事後検知**するが、`/goal` は**停止の手前で防ぐ**。
  両者は代替ではなく、**予防（`/goal`）と検知（`verify`）の二段**として使う。
- **`requirement.md` の「目的 / ゴール」とは別物**。あちらは永続する成果物（work の記録・retro が読む）、
  `/goal` はセッション内の揮発的なガード。**混同しない**——work のゴールを `/goal` に入れても、
  工程の停止条件としては粗すぎて機能しない。設定するのは**工程単位**のゴール。
- **工程を移るたびに更新する**（前工程のゴールが残っていると、次工程の停止判定に効かない）。
- autonomous では `humanGates` と併せて使う。ゲートが無い工程でも「記録まで終える」は変わらないため、
  むしろ自律実行時ほど有効。
