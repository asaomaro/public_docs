...
> **撤去（`aidev worktree rm`）は破壊的**——このハーネスで唯一「作業を失いうる」操作。
> `--force` は**未 push コミットを検知しない**し、`--delete-branch` は `git branch -D` 相当。
> 打つ前に `protocol-worktree.md`「撤去」を読むこと。
...
**条項 id は起票のその瞬間に生まれる。** 導入前の review.md にその id は決して現れないので、
**id 別の件数を前後で比べることはできない**（必ず `0 → N` と増える方向にしか動かない）。

そこで「前」は起票時に**観点で数えて**刻む。`--baseline` を必須にしているのはこのため。

- **変更を入れる前に、仮説と一緒に書く**ので、事後の物語作りにならない。
- **数えられないなら、その事実を値として書く**（`0件（review.md がまだ無く、前を作れない）`）。
  捏造も空欄も不可（「8.」の「欠落を捏造で埋めない」と同じ）。
  この場合は**前後比較が成立しない条項**なので、判定は根拠を明示して行う。

> **数え方が前後で違う点に注意**：`baseline` は人が review.md を**読んで**数え、
> 導入後は `[conv:<id>]` タグを**数える**。同じ観点を別の手段で数えているので、
> 境目の判断（何をその観点とみなすか）は `baseline` の値に文章で残しておく。
