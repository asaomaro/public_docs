...
- 刻印の無い work（直接 requirement から起こしたもの）は従来どおり検査しない。ただし
  **結果的に閉じた項目があれば deliver で反映する**のは同じ（散文ソフト規約。「2.6」の三層モデル）。

## 2.10 エージェント間の可搬性（Claude 固有機構のフォールバック）

**aidev は特定のエージェントに依存しない。** GitHub Copilot / Codex 等でも同じワークフローが回ることを
前提とする。Claude Code 固有の機構は**任意の高速化層**であり、無くても不変条件は保たれる。

### 採用基準

Claude 固有機構を使うのは、**同じことを「指示」で書けるものに限る**。指示で代替できない依存を作らない。

> **ハードな層（判定・不変条件）は必ず `aidev` CLI に置く。Claude 固有機構は CLI を呼ぶだけの層に留める。**

判定をフックやツール側のロジックに書くと、他環境で置き換えられなくなる。逆に CLI に置けば、
他環境では「そのコマンドを実行せよ」という一行の指示で等価になる。

### 規約には固有名を書かず「役割」で書く

エージェント組み込みのコマンド（レビュー・整理・スケジュール等）を実作業に使うのは構わないが、
**その固有名を規約に焼き付けない**。「2.5」が PJ 資産を固有名でなく役割で書いているのと同じ流儀。

- ❌ 「review 工程では `/code-review` を使う」——名前を書いた時点で他環境では意味を失う
- ✅ 「正確性・保守性の点検に使える**組み込みのレビューコマンドがあれば**使う。無ければジェネリック手順」

**「使えること」を不変条件にしない。** 組み込みコマンドやツールは環境によって存在しない
（実例: レビューの構造化出力ツールが不在で、テキスト報告に切り替わった）。したがって
**その出力形式や存在を前提にした成果物スキーマを作らない**——受け渡しはテキストを写し取る前提で設計する。

**組み込みコマンドは代替ではなく併用**。工程の観点のうち一部（正確性・保守性）しかカバーしないことが
普通で、要件適合・規約適合のような**その work の文脈を要する観点は工程 skill 自身が見る**。
「委譲」と扱うと観点が落ちる。

### 実作業の優先順位（三段階）

「2.5」の二段階を、組み込みコマンドを含む三段階に拡張する。

1. **PJ 固有の skill / コマンド**（AGENTS.md の観点を含む）——PJ の文脈を最もよく知る
2. **エージェント組み込みのコマンド**——汎用的な正しさの点検に強い。**役割で参照する**
3. **各工程 skill のジェネリック手順**——上記が無い環境でのフォールバック

いずれを採っても、ゲート・state 記録・遷移の制御は常に基盤（protocol）が担う（「2.6」）。
...
| 第二層（ハード） | `aidev` CLI（`guard` / `approve` / `verify`。sh と ps1） | **全環境**（CLI 無しなら手で同等に） |
| 第三層（自動化・任意） | Claude Code のフック等 | Claude Code のみ |

第三層は**第二層を自動で呼ぶだけ**で、判定を持たない。したがって外しても第二層が残る。

### フォールバック対応表

| Claude 固有機構 | 役割 | 他環境でのフォールバック | 失うもの |
|---|---|---|---|
| `Stop` フック | 停止前に `verify --strict` を強制 | 指示に「終える前に `aidev verify --strict`」を書く | 自動性のみ |
| `/goal`（「3.2」） | 停止前チェック（**ユーザーが設定**。エージェントからは設定できない） | 同じ内容を指示で書く | 自動性のみ |
| 組み込みレビューコマンド | 正確性・保守性の点検 | ジェネリック手順で自分で見る（下記「役割で書く」） | 網羅性 |
| plan モード（「10.」） | 探索 → 承認 → 実装 | 「先に方針を提示して承認を得てから書く」を指示で行う | 書き込みの機械的禁止 |
| `AskUserQuestion` | 選択肢 UX（承認ゲート） | 同じ選択肢をテキストで提示（`aidev-00-start`「3.」の既存規約） | UI の見やすさ |
| サブエージェント委譲（「2.6」） | 重い調査の隔離 | 同一セッションでインライン実行し、結論だけ残す | コンテキスト分離 |
| `aidev` CLI | state / metrics 更新 | 手で同等の編集を行う（各コマンドの記述に従う） | 検証と冪等性 |

**避けるべき依存**: 成果物の形式・不変条件そのものを Claude 固有機構に依存させること
（例: state を CLI 以外の機構で保持する、フックでしか生成できないファイルを前提にする、
Claude の出力形式に合わせて `metrics.yml` のスキーマを変える）。

### 他環境での設置

各エージェントのルールファイル（Codex 等は `AGENTS.md`、GitHub Copilot は
`.github/copilot-instructions.md`）に、次を書けば第三層の代替になる。
...
Claude Code でのフック設定は `aidev-docs/README.md`（任意のセットアップ）に置く。**グローバル設定に
暗黙に埋め込まない**——設定したマシンでしか効かない前提が不可視になるため、手順として明示する。
...
このハーネスで実際に起きた事故は「**やるべきことを終える前に停止する**」型が多い。`guard` と
`event start` が別コマンドである以上、記録漏れは構造的に起こりうる（実例: review の start が
両ラウンドとも欠落し、所要時間が導出できなくなった）。`aidev verify` は **deliver 前に WARN で
事後検知**するが、**予防**——停止の手前で止める——には別の層が要る。

**機械的な予防は Stop フックが担う**（`protocol.md`「2.10」第三層 / `aidev-docs/README.md` の任意
セットアップ）。`aidev verify --strict` を呼び、記録漏れなら停止をブロックして理由を返す。
フックが無い環境では、ルールファイルに「終える前に `verify --strict` を実行する」と書く（「2.10」）。

#### `/goal`（Claude Code・ユーザーが任意で設定）

`/goal`（*Set a goal Claude checks before stopping*）を併用してよい。ただし:

- **`/goal` は組み込みスラッシュコマンドで、エージェントからは設定できない**（モデルに公開された
  skill ではなく、ユーザーが打つもの）。したがって**工程 skill が自分で設定することはできない**。
  工程を案内するとき、必要なら**ユーザーに設定を促す**に留める。
- **設定する内容**: その工程の「完了の目安」＋**終了プロトコル（「3.」）の記録まで完了すること**。
  - 例（coding）: 「tasks.md の対象タスクを完了し、`aidev approve coding tasks_done=… unplanned_lookups=…`
    まで記録し終えている」
- **Stop フックとは範囲が違う**。フックは**記録漏れだけ**を機械的に見る。`/goal` はユーザーが任意の
  完了条件（記録以外の中身）を置ける。重なるが代替ではない。
- **工程を移るたびに更新が必要**（前工程のゴールが残ると次工程の停止判定に効かない）。
  この手間があるため、**記録漏れの予防はフックに任せ、`/goal` は必要なときだけ**使うのが実用的。
- **`requirement.md` の「目的 / ゴール」とは別物**。あちらは永続する成果物（work の記録・retro が読む）、
  `/goal` はセッション内の揮発的なガード。**混同しない**——work のゴールを `/goal` に入れても、
  工程の停止条件としては粗すぎて機能しない。
