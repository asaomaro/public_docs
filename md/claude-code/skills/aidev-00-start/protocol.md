...
2. **承認ゲート**：実行モード（「10. 実行モード」）に従う。
   - **interactive（既定）**: `AskUserQuestion` で承認と遷移を 1 問にまとめた単一4択を提示
     ...
     - ※**4つ目は条件付きで差し替えてよい**: 上流工程（requirement / spec / design / plan）で
       ...
       `独立検証を挟んでから決める`（「3.3」）を提示する。差し替えたときは理由を添える。
...
自動承認のため不要）。ただし上流工程で「独立検証」（「3.3」）を提示する場合は 4 つ目の枠をそちらに譲る
...
### 3.3 独立検証（成果物の内部一貫性を別コンテキストで点検・任意）

成果物を**書いた本人とは別のコンテキスト**に見せて内部一貫性を点検させる（自分の成果物の穴は
同じコンテキストからは見えにくい）。手戻りの最大の源は方向 / spec 誤りなので**上流工程で最も効く**。
...
- **規律**: **根拠（`file:line` / 節名）の無い指摘は出さない**。推測と事実を区別する
  （さもないと的外れな指摘でゲートが詰まる）。
- **結果の扱い**: 指摘あり → その場で直して再提示、または `差し戻す`（「3.」の分岐に合流）。
  指摘なし → 承認へ。**記録は通常ゲートと同一**（新しいメトリクスキーは設けない）。
- **使わない場面**: 自明な成果物／`profile: light`（往復を減らす趣旨に反する＝昇格の合図）／
  `autonomous` のゲート経路（工程内ステップとしては使ってよい）。
