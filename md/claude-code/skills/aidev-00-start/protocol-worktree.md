...
- **PJ 規約は並行作業でも変わらず適用される**: 共有ファイルへの波及や、検証を委譲せず主エージェントが行う
  ...
  **ただし deliver した時点で `inflight` から外れる**（掴んでいる人はもういないため）。
  未マージなら `todo` にも戻るので、**deliver 済み・未マージの区間だけは台帳から実態が読めない**。
  次に選ぶ前は `aidev worktree list` で未マージの枝を見ること。
...
- **いま重なっているファイルは `aidev worktree files` で見る**。`sharedFiles` は静的な宣言なので、
  「触らないファイルが警告され、触るファイルは警告されない」という反転が起きうる（実際に起きた）。
  `worktree files` は**未マージの worktree**の変更を突き合わせて、**2 本以上が触っているファイル**を出す。
  - **coding に入る前は `--planned`**（`tasks.md` の `対象:` を突き合わせる）。実差分は
    「もう書いた後」しか見えず、並行 N 本が同時に上流工程にいる立ち上がり期は**構造的に空になる**
    （実測 0 件 → deliver 前 5 件）。衝突を避ける余地があるのはその時間帯なので、そこでは**宣言**を見る。
