...
- **いま重なっているファイルは `aidev worktree files` で見る**。`sharedFiles` は静的な宣言なので、
  ...
  `worktree files` は未着地 work の実際の変更を突き合わせて、**2 本以上が触っているファイル**を出す。
  - **coding に入る前と、deliver の前**に見る。前者は衝突を避ける余地がまだあるうち、
    後者は「マージ順で相手を壊さないか」を判断するため。重なっていること自体は禁止ではない
    （並行可否はユーザー判断）——見ずに進むことだけが問題。
