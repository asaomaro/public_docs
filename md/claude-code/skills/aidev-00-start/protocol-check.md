...
- **フォールバック**: 委譲機構が無い環境では**同一セッションで観点を切り替えて読み直す**
  （コンテキスト分離は失うが点検項目は同じ）。**この場合は `approve coding` に
  `task_check_mode=same_session` を刻む**（委譲できたなら `delegated`）——点検が効く理由は
  コンテキスト分離なので、区別せずに足し上げると「別コンテキストが見て 0 件」と
  「本人が読み直して 0 件」が同じ数字になり、効果（「8.」）を横断で測れない。
