...
6. **`aidev coverage --strict` を通す**（exit≠0＝未解消の gap あり。目視確認で代替しない）。
   ...
   - **cover**（`AC:` 行の書き忘れ / タスクに落ちていない `AC`）: 分解の漏れなのでタスクを足す。
     **意図してこの work の外に出す AC がある場合は、`requirement.md` の `スコープ / 対象外` へ移す**
     （タスクを持たない AC を `完了条件` に残したまま承認しない——それは「完了条件を満たさずに完了できる」状態）。
...
- **`aidev coverage --strict` が exit 0**（`ac`/`spec`/`tasks` の 3 列が埋まり、gap が残っていない）。
