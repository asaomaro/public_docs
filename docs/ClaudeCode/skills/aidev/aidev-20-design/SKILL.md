...
## 依拠する既存の事実
- <この設計が前提にしている既存の挙動・構造と、**確かめた場所**（ファイル・関数名）>
（**推測で断定しない**。確かめていないものは「未確認」と書く。`doccheck` は出所の無い断定を指摘する）
...
- **依拠する既存の事実に出所が付いている**（付かないものは「未確認」と明記され、
  判断が要るなら `decisions.md` に残っている）。
