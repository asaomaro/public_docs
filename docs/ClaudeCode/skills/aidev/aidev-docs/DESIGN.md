...
- **Claude Code の plan モードは「探索して承認を取り、解除してから書く」形でのみ使う**：plan モードは
  ...
  - **「工程の間だけ plan モードにして、終わったら戻す」は作れない**（2026-09-06 に裏取り）。
    ...
    - **「plan モード」は製品固有名ではない**（Cursor / Copilot にもある横断的な概念名）ので、
      `Stop` フックや `AskUserQuestion` と違い CLI の出力に名指しで出してよい。
      持たない環境のフォールバックは `aidev-docs/README.md` の表。
...
| **3. サブエージェントでの実走** | テスト PJ で aidev フローを 1 本通す | lint も CI も「書いてあることの整合」しか見ない。**書いてある規約が実際に発火するか**は通り道を歩かないと分からない（分類 G） |
