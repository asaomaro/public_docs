...
**方針の事前承認（成果物を書く前に方針だけ承認を得る）の条件の正典は
`protocol-autonomous.md`「方針の事前承認」の 1 箇所**。基準・対象工程・前提は**すべてそこ**にある。
**どこにも写さない**（写した条件は次の変更で取り残される——実際に 5 回起きた。
...
**持っているかは製品単位ではなく「サーフェス単位」で見ること**。同じ製品でも CLI と IDE で違い、
**同じ Claude Code でも主エージェントとサブエージェントで割れる**（サブエージェントには
主エージェント専用のツールが渡らない）。この表は「その環境に無いとき何をするか」の一覧なので、
**製品名ではなく手元のサーフェスで引く**。

> **plan モード対応は 2026-09-07 に廃止した**ので、この表から落ちている。規律（方針を先に承認させる）は
> `protocol-autonomous.md`「方針の事前承認」に残り、**どの環境でも同じ散文で効く**。
> 廃止の経緯と、そのとき調べた各社の plan モードの実態は `DESIGN.md`「2.」。
