...
判断基準は 2 つ——(a) 上流のゲートが同じ判断を承認済みでないこと、(b) plan モードが買う
「**コード探索中の read-only 強制**」が要ること。**入るのは `spec` / `design` / `plan`**
（入力に既存コードが入る工程）。`requirement` は主活動がヒアリングなので (b) が要らない。
**この基準には前提がある**（既存のゲートが機械で止まること）。崩れると逆の結論が出るので、
**いつ読み直すかまで含めて `protocol-autonomous.md` に書いてある**（前提の欠落は lint で検査できない）。
...
| plan モード（**固有ではない**。Codex CLI は `/plan`・Shift+Tab、Copilot は VS Code / Visual Studio の Plan agent。持たない環境向けの行） | 「先に方針を提示して承認を得てから書く」を指示で行う。**入る条件は `protocol-autonomous.md`**（ここに写さない）——なお plan モードを持たない環境は、その基準の前提が崩れる側なので特に読み直すこと |
