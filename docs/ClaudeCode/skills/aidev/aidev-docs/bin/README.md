...
| **L9 判定条件の写し** | 工程 SKILL の **plan モードに言及する行**が、判定に使う語（`profile` / `humanGates` / `interactive` / `autonomous` / `full ×` / `light`）を抱えていないか。**条件を skill に写した時点で、次の変更での取り残しが予約される**——実際にそれが起きた（`guard` の条件を「承認者がいるか」へ変えたとき、CLI と散文は直したのに skill の `full × interactive` が残った）。規律自体は既にあった（`doccheck` は「正典は protocol-check。ここは引き金と打つコマンドだけ」）ので、**規律はあったのに守られなかった**型として観測点にした。<br>**判定キーの有無だけでは足りない**——取り残された文言は `full` × `interactive` で**キー名を 1 つも含んでいなかった**ので、値の側も見る。対象を「plan モードの行」に絞るのは、`profile: light` 等の正当な言及が工程 SKILL に 13 ファイルあるため（全部を弾くと誤検知だらけになる）。正典への参照（`protocol-autonomous.md` というファイル名が `autonomous` を含む）は落としてから判定する。 |
