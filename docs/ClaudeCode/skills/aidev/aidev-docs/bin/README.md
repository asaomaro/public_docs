...
| `guard <phase>` | 工程開始時の**前提チェック**（前提成果物の有無・前提工程の承認・`dependsOn` 充足）。未充足なら非ゼロ終了。**subtask では `plan`/`coding`/`test`/`review` 以外を拒否**する（それ以外は親 work 専用。exit 2）。OK のときは `aidev event <phase> start` を忘れないよう促す。**phase 以外の引数は弾く**（exit 1）——`taskcheck` / `doccheck` / `debug` が `--slug` を取るので、その癖のまま `aidev guard spec --slug X` と打てる。黙って捨てていた頃は **`.aidev/current` の別 work に対して緑を返していた**（工程入口の硬ゲートが打ち間違いを通す形。sh / ps1 の**共有欠陥**なのでパリティテストでは捕まらなかった）。<br>**`spec` / `design` では plan モードへ入るよう促す**。散文（`protocol-autonomous.md`）に「この条件なら入れ」と書いてあっても、**打つ側は工程に入る瞬間には思い出さない**——`event start` の促しと同じ型で、CLI が散文を肩代わりする。**条件の判定を CLI に置く**のが要点で、散文だけに置くと勧めてはいけない場面で勧めてしまう事故を機械が止められない。<br>**見るのは「その工程に承認者がいるか」（`has_approver`）で、`mode` そのものではない**——plan モードを抜けるのが人間の承認だから。`mode != autonomous` で判定していた頃は、`humanGates: [spec]` の部分自律（他 PJ の retro が実績として報告している構成）で**承認者がいるのに促しを止めていた**（実走で実測）。`light` だけは承認者の有無と無関係に外す（往復を減らす趣旨に反する）。なお `humanGates` は `new` が書くだけで**どこからも読まれていなかった**——書いて忘れられた設定の 1 例。<br>**承認者がいない工程で入ると抜けられない**（書き込みが塞がったまま工程が完走できない）ので、促さないことが安全側になる。<br>**文面は「入れ」と命じる**——「方針を先に固める」のような役割だけの言い方だと、**丁寧に計画するだけでモードは切り替わらない**（`EnterPlanMode` は主エージェントが持つツールなので、明示すれば実際に切り替わる）。「plan モード」は Cursor / Copilot にもある横断的な概念名なので、`Stop` フックのような製品固有 API 名と違い CLI に名指しで出してよい（`DESIGN`「2.」に裏取り）。<br>**条件は工程 SKILL に写さない**（正典は `protocol-autonomous.md`）。写した条件は次の変更で取り残される——実際、`guard` の条件を「承認者がいるか」へ変えたとき spec / design の SKILL に写した `full × interactive` が残り、**部分自律で CLI は促すのに skill は「入るな」と読める**状態になった。`lint-docs.sh` の **L9** が「工程 SKILL の plan モード行が判定条件を抱えていないか」を検査する。 |
...
| `doccheck <start\|report\|status> ...` | **上流4文書（requirement / spec / design / plan）の独立点検を有限化する**（`protocol-check.md`「(a)」）。`taskcheck` と同じ「1ラウンド = 1 対」の形で、**同じ理由**——(a) は長く**観測点の無い規約**で、`autonomous` では「使わない場面」に入っていたため、上流文書は一度も点検されないまま承認できた。<br>`start <phase> --mode <delegated\|same_session> [--slug s]` が `maxDocCheckRounds`（既定 2・下限 1）を検査し、超えていれば **exit 4**。対象の md が無ければ弾く。出力は「渡すもの／内部一貫性だけという観点／**外部ソース照合の禁止**／固定の返却形式」を毎回示す。<br>`report <phase> --findings <n>` が結果を受ける。`--findings` 必須、**`start` と対でだけ受ける**（`taskcheck` と同じ穴を塞いである）。`doc_check_rounds` も `taskcheck` と同じく **`report` まで届いたラウンドだけ**数える。<br>**`report` は「直す前に」打つ**という規約には観測点が無く、破っても何も起きなかった（他 PJ の retro で実際に 1 回発生し、件数が次のラウンドに寄った）。`start` が対象 md の**バイト数**を刻み、`report` で変わっていれば「直したあとに打った」と言う。**`plan` は `plan.md` と `tasks.md` の合計**——`start` の出力自身が「渡すもの: plan.md と tasks.md」と言うのに `plan.md` しか見ておらず、`AC:` 行がある（＝plan の点検で最も直しやすい）`tasks.md` だけを直しても何も言わなかった（実走で実測）。ハッシュでなくバイト数なのは **sh と ps1 で同じ値を出せる最も単純な量**だから——同じ大きさの書き換えは見逃すので、**`note:` 止まりで exit code は変えない**（見逃す検査を硬ゲートにしない）。<br>`status [--format table\|tsv]` は工程ごとの rounds / findings / mode / 上限到達 / `unreported`。**未報告の検知は `taskcheck` と同じ関数・同じ文言・同じ出口**（`decisions.md` に工程名を含む行があれば消える）——`protocol-check.md` は (a) と (b) で「規律・禁止事項・フォールバックは共通」と宣言しているのに、数え方だけ対称化して検知は (b) にしか無く、**上流文書の「委譲を出し忘れた点検」はどのコマンドを打っても一生表に出なかった**（実走で実測）。未報告の `start` も `maxDocCheckRounds` の枠は食うので、黙っていると「静かに枠だけ減って次の `start` が exit 4」という詰まり方をする。**`at_max` 列が見るのは `start` の数**（`rounds` 列に出す `report` 数で判定すると、「まだ余裕がある」と出したそばから次の `start` が exit 4 になる）。**`profile: light` は対象外**（往復を減らす趣旨に反する）。<br>**副産物**: `approve <phase>` が `doc_check_rounds` / `doc_check_findings` / `doc_check_mode` を**この記録から自動で刻む**（`taskcheck` と同じ理由）。schema 11 では `autonomous` でこの記録が無いと `verify` が WARN、`--strict` で致命。<br>**subtask も対象**——`coverage` / `smoke` が「家族の根だけ」なのと違い、点検は**その work が自分で書いた md** に対して働く。subtask は `requirement` / `spec` / `design` を親から継承する（子には md が無い）ので、実際に検査されるのは**子自身の `plan.md` だけ**になる。 |
...
- schema ≥ 11 の検査: **`autonomous` の上流4文書は独立点検の記録を持つ**（`requirement` /
  `spec` / `design` / `plan`。承認済みかつその md が実在する工程だけ。`profile: light` は対象外）。
  ...
  「手戻りの最大の源は方向 / spec 誤り＝**上流で最も効く**」と書いているのに優先順位が逆で、
  他 PJ の retro が「4 工程すべてで差し替えを提案せず、spec の自己矛盾を素通りさせた」と実測した。
  人間の目が入らない以上、点検の要否を**書いた本人の裁量に委ねない**。
  **subtask では子自身の `plan.md` だけが対象**（上流3文書は親から継承するので子には md が無く、
