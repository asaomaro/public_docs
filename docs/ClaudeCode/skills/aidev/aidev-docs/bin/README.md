...
| `guard <phase>` | 工程開始時の**前提チェック**（前提成果物の有無・前提工程の承認・`dependsOn` 充足）。未充足なら非ゼロ終了。**subtask では `tasks`/`coding`/`test`/`review` 以外を拒否**する（それ以外は親 work 専用。exit 2）。OK のときは `aidev event <phase> start` を忘れないよう促す。**phase 以外の引数は弾く**（exit 1）——`taskcheck` / `doccheck` / `debug` が `--slug` を取るので、その癖のまま `aidev guard design --slug X` と打てる。黙って捨てていた頃は **`.aidev/current` の別 work に対して緑を返していた**（工程入口の硬ゲートが打ち間違いを通す形。sh / ps1 の**共有欠陥**なのでパリティテストでは捕まらなかった）。<br>**該当する工程では plan モードへ入るよう促す**（**どの工程かも条件も `protocol-autonomous.md` が正典**——ここに写さない。写した版は 5 回とも取り残された）。散文（`protocol-autonomous.md`）に「この条件なら入れ」と書いてあっても、**打つ側は工程に入る瞬間には思い出さない**——`event start` の促しと同じ型で、CLI が散文を肩代わりする。**条件の判定を CLI に置く**のが要点で、散文だけに置くと勧めてはいけない場面で勧めてしまう事故を機械が止められない。<br>判定は `has_approver()` に切り出してある（部分自律を取りこぼしていた回の産物。**その設定は `new` が書くだけでどこからも読まれていなかった**——書いて忘れられた設定の 1 例）。<br>**文面は「入れ」と命じる**——「方針を先に固める」のような役割だけの言い方だと、**丁寧に計画するだけでモードは切り替わらない**（`EnterPlanMode` は主エージェントが持つツールなので、明示すれば実際に切り替わる。ただし**入れるのは Claude Code の主エージェントだけ**で、Codex / Copilot は人が切り替える——サーフェス別の一覧は `aidev-docs/README.md`）。「plan モード」は Cursor / Copilot にもある横断的な概念名なので、`Stop` フックのような製品固有 API 名と違い CLI に名指しで出してよい（`DESIGN`「2.」に裏取り）。<br>**条件はどこにも写さない**（正典は `protocol-autonomous.md` の 1 箇所）。写した版は**3 回とも次の変更で取り残された**ので、`lint-docs.sh` の **L9** が検査する。<br>**`profile: light` では `design` / `tasks` / `research` / `architecture` を exit 2 で弾く**（light は上流を requirements 1 ゲートに畳む）。長く散文だけの規約で、CLI は通したうえに「忘れずに `aidev event design start`」と**打つよう促していた**（実走が実測）。`verify` の事後 WARN と対で、入口でも止める。 |
...
| **L9 判定条件の写し** | **実行時文書＋ `aidev-docs/*.md`** の **plan モードに言及する行**が、判定に使う語（`profile` / `humanGates` / `interactive` / `autonomous` / `full ×` / `light`）を抱えていないか。**条件を skill に写した時点で、次の変更での取り残しが予約される**——実際にそれが起きた（`guard` の条件を「承認者がいるか」へ変えたとき、CLI と散文は直したのに skill の `full × interactive` が残った）。規律自体は既にあった（`doccheck` は「正典は protocol-check。ここは引き金と打つコマンドだけ」）ので、**規律はあったのに守られなかった**型として観測点にした。<br>**判定キーの有無だけでは足りない**——取り残された文言は `full` × `interactive` で**キー名を 1 つも含んでいなかった**ので、値の側も見る。対象を「plan モードの行」に絞るのは、`profile: light` 等の正当な言及が工程 SKILL に 13 ファイルあるため（全部を弾くと誤検知だらけになる）。正典への参照（`protocol-autonomous.md` というファイル名が `autonomous` を含む）は落としてから判定する。<br>**走査対象に `aidev-docs/*.md` を明示的に足すのが要点**——初版は「実行時文書ぜんぶに広げた」と書きながら `runtime_docs` を使っていたので、**旧条件が実際に残っていた `DESIGN.md` に永久に届かなかった**（`README.md` に残っていた 2 件も同じ理由で機械に一度も見えていなかった）。`runtime_docs` と SKILL のグロブは全工程 SKILL で重なるので**和集合を取ってから走査する**——重ねたまま回すと 1 件の写しが「2 ファイル」と出る（`runtime_docs` 自身のコメントが同じ罠を警告しているのに、その隣で再発させた。`runtime_docs` は `$d/SKILL.md` を出すので `//` を潰さないと `sort -u` が効かない）。<br>**`DESIGN.md` は「なぜその基準にしたか」を書く場所**なので規則に触れる行が正当に在る。全部弾くとノイズ、全部許すと旧条件が生き残るので、**L5 と同じ形——`lint-docs.allow` の `L9:` 行に理由つきで登録した行だけ免除する**。 |
...
3. **サブエージェントでの実走** — テスト PJ で aidev フローを 1 本通す。
   **射程は第二層まで**——サブエージェントには `EnterPlanMode` のような主エージェント専用ツールが
