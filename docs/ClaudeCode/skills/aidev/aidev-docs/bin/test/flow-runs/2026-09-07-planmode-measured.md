...
### 2 本目（承認オプションを変えて）— 測れないことが分かった

1 本目の「残す既知の穴」（承認者が別のオプションを選んだ場合）を、`20260907-import-mapping`
（`mode: autonomous` / `humanGates: [design]`）で測りに行った。`guard design` が促しを出すこと
——`has_approver` が humanGates を見ていること——も併せて確認。

**結果: 測れない。** `ExitPlanMode` の出口メッセージは承認オプションに関わらず**完全に同一**で、
**抜けた側からモードを観測する手段が無い**。分かるのは最初の書き込みを試したときだけ。
2 本目も `design.md` → `doccheck` → `coverage`（3/3）→ `approve design` が許可要求なしで通ったが、
これは「オプションで変わらない」の証明にはならない——**区別できないから**。

→ 正典に書けるのは「**抜けたら真っ先に成果物を 1 つ書く**（そこが唯一の観測点）」までにした。
「どのモードに着いたか」は**確かめる手段が無いので書かない**。

**副産物: plan file は work 単位ではなくセッション単位**。2 本目で入ったとき
`A plan file already exists at …` と出て、**1 本目（CSV エクスポート）の下書きが残っていた**。
しかも指示は「incremental edits」——追記を促してくる。素直に従えば**別 work の設計が混ざった
plan file** を承認者が読むことになる。→「**入ったら全面的に書き換える**」を明記した。

**残す既知の穴**: 承認オプションごとの着地先は、**この経路では原理的に測れない**。
測るならエージェント側ではなく、ホスト（Claude Code 本体）の設定を観測する経路が要る。
