...
# 3460 -> 3467: **他 PJ の retro（server）の H5** を入れたぶん（+7、`protocol-check.md`「(b)」）。
#   「最小の差分で当てる」は既に**共通節**にあり (a)(b) 両方に掛かっていた（retro は「(a) だけ」と
#   書いていたが**その前提は誤り**）。足りなかったのは別のこと——**規則そのものを書き直したときは、
#   書き直した規則を新規実装として点検し直す**。書き直しの根拠に置いた前提のほうが誤っていて、
#   退行を作った実測がある（差分は小さくても判定の全体が変わるので、最小の差分の話とは別）。
# 3467 -> 3471: **retro の「製品 / コード」提案を backlog に落とす導線**（+4、`aidev-95-retro`）。
#   口が `new|archive|compact` しか無く、既存ファイルに 1 行足す手段が無かったので
#   `backlog add` を足し、skill 側にも「**同じコミットに含める**」を書いた。
#   条項が `convention new` で機械的に起票されるのと同じ扱いにする——散文の提案のままだと
#   **次の work に届かない**（予告された欠陥が backlog に入らず予告どおり再発した実測がある）。
# 3471 -> 3477: **`idle_sec` の禁止事項を読まれる場所へ**（+6、`protocol.md`「8.」）。
#   実走が「列を見ただけでは意味も禁止事項も決まらない」と実測した——唯一の禁止事項
#   （**`work_sec` から引かない**）が `bin/README.md` の巨大な 1 セルにしか無く、
#   retro を書く立場が `metrics` を読む導線からはそこへ辿り着かない。**規約は読まれる所に置く**。
# 3477 -> 3485: **独立監査の should 2 件**（+8）。`idle_sec` を足しても**横断比較の当事者**
