...
- **フォールバック**: 委譲機構が無い環境では**同一セッションで観点を切り替えて読み直す**
  （コンテキスト分離は失うが点検項目は同じ）。**その場合は `taskcheck start --mode same_session`**
  （委譲できたなら `delegated`）——点検が効く理由はコンテキスト分離なので、区別せずに
...
- **`aidev doccheck start <phase> --mode <delegated|same_session>` を打ってから委譲する**。
  ラウンド上限（`maxDocCheckRounds`。既定 2）を CLI が検査し、超えていれば exit 4 で止まる。
  結果は `aidev doccheck report <phase> --findings <件数>` で記録する（(b) と同じ形）。
- **結果の扱い**: 指摘あり → その場で直して再提示、または `差し戻す`（「3.」の分岐に合流）。
  指摘なし → 承認へ。**上限で止まったら深追いしない**——残った疑問を `decisions.md` に残して
  承認へ進み、判断は次工程・60 review に委ねる。
- **`mode: autonomous` では必須**（人間の目が入らないので、点検の要否を書いた本人の裁量に
  委ねない）。**coding が全タスク必須なのに上流が任意**だと、同じ文書が「上流で最も効く」と
  書いている優先順位と逆になる。承認済みなのに記録が無ければ `verify` が WARN、`--strict` で致命。
- **使わない場面**: 自明な成果物／`profile: light`。`interactive` では従来どおり任意
  （承認ゲートの 4 つ目、または工程内ステップ）。
