...
- **書く順序**: **plan モードは自前の plan file を持つ**（`ExitPlanMode` は内容を引数に取らず、
  ...
  **plan file は work 単位ではなくセッション単位**——同じセッションで 2 本目の work が入ると
  **1 本目の下書きが残ったまま**「追記せよ」と促される（実測）。**入ったら全面的に書き換える**。
  混ざったまま承認を求めると、承認者は別 work の設計を読むことになる。
- **抜けた先のモードは、抜けた側からは観測できない**（実測）。`ExitPlanMode` の出口メッセージは
  承認オプションに関わらず同一で、分かるのは**最初の書き込みを試したとき**だけ。
  だから**抜けたら真っ先に成果物を 1 つ書く**（そこで許可を求められるかが唯一の観測点）。
  「元のモードに戻らない」は書けるが、「どのモードに着いたか」は書けない——確かめる手段が無い。
- **入った先には別の手順書が待っている**（2026-09-07 に主エージェントで実測）。plan モードは
  「Explore サブエージェントを最大3本 → Plan エージェント → レビュー → plan file → ExitPlanMode」という
  **自前の 5 段の手順**を注入し、「ターンは `AskUserQuestion` か `ExitPlanMode` でしか終われない」まで課す。
  **工程の手順と成果物は aidev 側が正典**——注入された手順から使うのは**探索と plan file だけ**にする。
  plan file には**その工程の成果物の下書き**を書き（design なら `design.md` の中身）、抜けてから清書する。
  サブエージェントの本数は「2.6」の委譲規約に従う（小さな work で 3 本立てない）。
- **`EnterPlanMode` を持っていても即座に呼べないことがある**（同日に実測）。`allowed-tools` に書いてあっても
  遅延読み込みの環境ではスキーマ取得が要る。**呼べなければ促しに従えない**ので、その場合は
  フォールバック（`aidev-docs/README.md` の**フォールバック表**）に落ちる——**入れないこと自体は工程の失敗ではない**。
- **そもそもエージェント自身の入口が無い実行主体がある**（Codex CLI・Copilot がそう。
  **Claude Code でもサブエージェントは入口が無い側**——製品ではなく**実行主体で分かれる**。
  サーフェス別の一覧は `aidev-docs/README.md` の**サーフェス表**）。そこでは上の基準は「入るか」ではなく
  **「承認を挟むか」として読む**——`guard` の促しは**人への依頼文**になり、従えなくても失敗ではない。
...
- **「入れ」と明示する**——「方針を先に固める」のような言い方では丁寧に計画するだけで
  ...
  **エージェント自身の入口が無い**実行主体は `aidev-docs/README.md` のサーフェス表）。`aidev guard design|architecture|tasks` が該当条件のときだけ促す。
