...
- **`EnterPlanMode` を持っていても即座に呼べないことがある**（同日に実測）。`allowed-tools` に書いてあっても
  ...
  フォールバック（`aidev-docs/README.md` の表）に落ちる——**入れないこと自体は工程の失敗ではない**。
- **そもそもエージェント側の入口が無い環境がある**（Codex CLI・Copilot は**人が切り替えるものだけ**。
  サーフェス別の一覧は `aidev-docs/README.md`）。そこでは上の基準は「入るか」ではなく
