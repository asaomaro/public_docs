# 実行モード（autonomous）と方針の事前承認（`protocol.md`「10.」の詳細）

> **読む条件**: `state.yml` が `mode: autonomous` の work／`aidev-util-batch`／方針を先に承認させるか迷ったとき
> 核となる規約と要約は `protocol.md`「10.」。ここはその詳細（工程ごとの読み込み量を抑えるため切り出し）。

`state.yml` の `mode` で実行モードを選ぶ。新規作業時に決定する（既定 interactive）。

### interactive（既定）
- 各工程末で人間の承認ゲート（「3.」）を通す。自動遷移しない。

### autonomous（夜間自律・PRまで一気通貫）
人間ゲートを置かずに requirements→…→deliver を自律実行し、**PR を出して停止**する。
「ゲートを消す」のではなく「**ゲートを PR（最終レビュー）に集約し、自己チェックを固くする**」モード。

- **requirements**: 起動時に与えられたタスク指示を requirements とする（対話ヒアリングはしない。
  指示が不十分なら autonomous を中止し interactive を促す）。
- **任意工程（research/architecture）**: 推奨ではなく**自律的に採否を決める**。
  発火条件の正典は `protocol.md`「4.5」で、autonomous でもそこを変えない——**条件に当たったら実施、
  当たらなければ実施しない**（「autonomous だから全部やる」ではない。同じ条件から逆の判断が出ないように、
  ここに独自の既定を置かない）。**`walkthrough.md`（レビュー補助）も同じ扱い**——要否は
  `aidev-60-review`「レビュー補助」の 3 条件だけで決める。**当たらなければ書かない**
  （「PR を出すから書く」と足すと、正典と逆の結論が出る）。
- **承認ゲート**: 自動承認（「3.」の autonomous 分岐）。`humanGates` に指定された工程だけは人間に確認
  （**部分自律**。例: `humanGates: [design]` で方向性の誤り＝最大の手戻り源だけ人間が止める）。
- **終端**: deliver は **PR を作成して停止**する。**auto-merge は禁止**（マージは人間が行う）。
  - **PR を作れない環境**（remote が無い／PJ が PR 運用でない／ユーザーが PR を禁じた）では、
    **作業ブランチへのコミットで停止**し、「どこに何を着地させたか（ブランチ名・コミット）」と
    「人間のレビューが未実施であること」を報告に明記する。ここを書かないと、着地点が無いまま
    手順を埋めようとして**存在しない規約を引用する**（実際に起きた）。
  - この場合 **`review.md` の「PR レビュー（人間）」節は生まれない**。条項・ハーネス改修の効果検証は
    AI の出力だけを材料に回ることになるので、insights はその旨をレポートに明記する
    （`protocol-conventions.md`「効果を判定する」の人間信号の優先はここで効かなくなる）。

### 安全弁（autonomous 必須）
- **テストを硬いゲートに**: test が通らないまま PR を出さない。未解決なら **draft PR** にして要点を報告。
- **ループ上限**: review/test→coding の差し戻しは **`state.yml` の `maxSendBacks`（未指定なら 3）回まで**
  （同一工程あたり）。現在値は `metrics.yml` の当該 phase の `sent_back` イベント件数
  （`by: unapprove` を除く）で判定する。除かないと、一度も失敗していない coding / test が
  上流の取り消しだけで予算を使い切る。
  上限到達後は**差し戻しを続けない**。まず `aidev debug start` で**まっさらなコンテキストへ原因究明を
  委譲する**（「10.」／`protocol-debug.md`）——上限は回数を止めるだけで方向を変えないため。
  デバッグも尽きた（`stop_for_human`）なら**停止し、未解決点を報告して人手に委ねる**
  （test が未通過のままなら deliver は draft PR とする）。
- **予算/時間上限**: 上限到達で停止・報告（無限ループ防止）。
- **記録継続**: モードに関わらず state/metrics/各成果物・`walkthrough.md` は残す（朝の一括レビューの証跡）。
- **逸脱記録**: 自律中の重要判断は decisions.md に残す。
- **独立点検**: 人間の目が入らないので、上流4文書は `aidev doccheck`、coding の各タスクは
  `aidev taskcheck` で点検を記録する（`protocol-check.md`「3.3」）。記録が無いまま承認すると
  `verify` が WARN（**致命になるのは上流4文書の欠落だけ**——`--strict` で exit 5）。

### 実行手段（別レイヤ）
autonomous の「夜間に回す」には実行主体が要る（headless 実行 / スケジュール起動でオーケストレーターが
本 skill 群を駆動）。これは harness（プロセス定義）とは別レイヤで用意する。

### 方針の事前承認

**有力案が複数あるときは、成果物を書く前に方針だけを提示して承認を得る**（承認されてから書く）。
採らなかった案と理由は `decisions.md` に残す——**選んだ結果しか残らないと、後から選び直せない**。

- **対象は `design` / `architecture` / `tasks`**。根拠は**成果物が「複数の案から選ぶもの」であること**
  ——design は方針、architecture は構造、tasks は分け方で、どれも選ぶ余地がある。
  `requirements`（何を・なぜ）は選ぶ工程ではなく、test・review・deliver・retro は
  判定・指摘・着地・振り返り。**`light` は除く**（往復を減らす趣旨に反する）。
  **subtask の `tasks` も除く**（切り方は親の tasks が確定済み。`aidev-30-tasks`）。
- **方針は `decisions.md` に書いて提示する**。採否と理由が同じ場所に残るので**承認の対象が後から辿れる**
  ——会話だけで済ませると記録に残らない（それは廃止した plan モードの欠点をそのまま引き継ぐ形）。
- **承認者がいる判定でも、返事が返らないことがある**（無人で回す `autonomous` × `humanGates`）。
  そのときは**方針を `decisions.md` に書いて先へ進む**——承認は**既存の approve ゲートが受ける**ので、
  ここで止める必要はない。**黙って承認済みとして扱わない**（書いた事実が残ればゲート側で拾える）。
- **その工程に承認者がいることが要る**——承認を出すのは人間なので、いなければ待つ相手がいない。
  `autonomous` でも `humanGates` に挙がっていれば承認者はいる（`has_approver()`）。
  承認者がいない工程では、方針も成果物も**同じ approve ゲートで一度に受ける**。
- **`aidev guard design|architecture|tasks` が該当条件のときだけ促す**。判定を CLI に置くのが要点で、
  散文だけに置くと、勧めてはいけない場面で勧めてしまう事故を機械が止められない（`protocol.md`「2.5」）。
- **aidev のゲートを置き換えない**。方針の承認と成果物（`design.md` など）の承認は対象が違うので、
  方針の合意を取っても `approve design` は省かない。
- 「工程内で段階的に確認したい」は **段階レビュー（「3.1」）**が対応する。
- **書き込みが塞がるモードで起動されていたら、工程に入る前に抜けるよう促す**（工程を中途で失敗させない）。
  CLI はモードを検知できないので、**`autonomous` を回す環境の既定モードで書き込みを塞がないこと**
  （Claude Code なら `defaultMode: plan` がそれ）——書き込みが塞がったまま完走できない work ができる。

> **かつてここには Claude Code の plan モードを使う規律があった（2026-09-07 に廃止）。**
> エージェント自身が入れるのは Claude Code の主エージェントだけで**移植できず**、入った先の手順が
> 工程の手順と競合し、**承認対象（plan file）も metrics も残らなかった**。
> **規律は残して、製品固有の機構への依存だけ捨てた**——経緯は `aidev-docs/DESIGN.md`「2.」。
