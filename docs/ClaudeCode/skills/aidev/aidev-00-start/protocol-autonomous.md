...
- **判断基準は「二重ゲートに見合うか」**。aidev は全工程に承認ゲートを持つので plan モードは
  2 つ目のゲートになる。**plan モードで承認するものが、aidev のゲートが承認するものと違うとき**
  だけ割に合う。禁じられるのは Write / Edit だけで対話は動くので、「実装計画かどうか」は基準にならない。
- **入る**: `aidev-00-start` の三層判定（aidev のゲートがまだ無い＝二重にならない。解除後
  `aidev new` へ引き渡す）／spec・design で有力案が複数あるとき（承認するのは**方針**、
  ゲートが承認するのは**文書**＝別物。`light` 以外で、その工程に**承認者がいるなら**——
  `autonomous` でも `humanGates` に挙がっていれば承認者はいる）／ハーネス自体の改修（aidev の対象外）。
- **入らない**: 上を満たさない全工程。**requirement / plan / test / review / walkthrough / deliver /
  retro は成果物がその工程の判断そのもの**で、ゲートが同じものを承認する（探索が要るなら
  三層判定で先に済ませる）／coding は `tasks.md` が承認済みの計画で、方針変更は**差し戻し**を使う
  （差し戻しは metrics に残る）／research は「2.6」の委譲が正／`profile: light`（往復を減らす
  趣旨に反する）／承認者がいない工程（下記）。
