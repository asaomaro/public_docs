...
2. 差分を以下の観点で点検する。**実作業の優先順位は protocol.md「2.5」の三段階**に従う
   ...
   - **ラウンド 2 以降は (a) 前ラウンドの指摘の解消と (b) そのラウンドの差分の must/should に絞る**。
     周辺の**以前からある**欠陥は backlog へ回し、差し戻さない（`aidev backlog add`）。絞らないと
     ラウンドごとに範囲が広がって**収束しない**（実測：review が 7 ラウンド。絞って初めて閉じた）。
