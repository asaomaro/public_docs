...
**ビューアがバンドルを受け取る口は 4 つ**（**どれもファイルを実行しない**）:

| 口 | 誰が使う | いつ |
|---|---|---|
| ファイル選択 / ドラッグ＆ドロップ | 人間 | 開いたあと、いつでも |
| 埋め込み（`html` の出力） | 通常の 1 枚 HTML | 起動時 |
| `window.__DIFF_REVIEW_BUNDLE__` | ホスト（エディタ拡張など）が**自分で書いたとき** | 起動時 |
| `postMessage` | ホスト・親フレーム | いつでも |

> [!NOTE]
> **生成物が外部ファイルを読み込むことはありません。**
> `file://` では `fetch` も `XMLHttpRequest` も使えず、隣のファイルを読む手段は
> `<script src>` だけで、それは**そのファイルを実行する**ことでもあります（実測）。
> 読む手段を持つと「どのバンドルを読むのか」がファイル名任せになり決まらなくなるうえ、
> 他人から受け取った `.dreview` を実行することにもなるので、**口ごと持ちません**。
...
- **`file://` では `fetch` / `XHR` / モジュール import が使えない**（実測）。
  ...
  ホストからの受け渡し（`postMessage` / 変数の注入）に限る。**どれもファイルを実行しない**。
...
| `view --readonly` で作ったビューア HTML を webview に流す | 差分を持たないので**どのバンドルにも使い回せる** |
| バンドルを `postMessage` で渡す（または流し込む HTML に `window.__DIFF_REVIEW_BUNDLE__ = …` を足す） | 後者は起動時から中身が出るので、案内が一瞬見えない |
