...
> [!NOTE]
> **表示される中身は「その HTML の中身」と「人が選んだファイル」だけで決まります。**
> 外部ファイルを読む口（`<script src>`）も、外から押し込む口（`postMessage`・グローバル変数）も
> 持ちません。前者は `file://` で唯一動く手段でしたが、**どのバンドルを読むのかが
> ファイル名任せになって決まらなくなる**うえ、そのファイルを実行することにもなります。
> 後者は、同じ HTML を開いても外からの操作で表示が変わりうる形になります。
...
- **開いたあとに外から中身を差し替えることはできない**（`postMessage` の受け口を持たない）。
  ホストが表示を変えるときは、HTML を組み立て直して渡す。
