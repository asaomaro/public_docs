...
## ユーザー要望（残り20行以下では「すべて表示」以外のボタンを出さない）

- [nit][conv:-] `redrawFile()` のフォーカス復元は、クリックした「↑」/「↓」が
  再描画後に無くなっていた場合、同じ隙間の「すべて表示」へではなく「ファイル内
  最初の展開ボタン」へフォールバックする。今回の変更で「↑」/「↓」が消える
  遷移がこれまでより起きやすくなったため、マウス操作のみのユーザーでは無関係な
  隙間へフォーカスが飛ぶ頻度がわずかに増え得る / 対応: 見送り（既存の
  フォールバック連鎖自体は今回の変更前から同じ仕組みで安全に動作しており、
  今回の変更が壊したものではない。UXの微調整であり、必要になれば別途対応する）。

独立点検（別コンテキストの subagent）で must/should の指摘は無し。
`remaining === EXPAND_STEP`（20）ちょうどの境界で `↑`/`↓`/`すべて表示`
いずれを押しても結果が完全に一致すること（`remaining === 21` では2回必要になる
のと対比して `<=` が正しい演算子であること）、「すべて表示」だけが残る形でも
`shown.bottom` が残留していても `remaining` の再計算に影響しないこと、
`redrawFile()` のフォーカス復元フォールバックが「展開帯まるごと消える」
既存ケースの部分集合として安全に動くこと、を確認済み（decisions.md D27 参照）。
`python3 -m unittest`（140件）も green のまま。

## ユーザー報告（展開ボタンクリック後に表示位置が動く）

- [nit][conv:-] `anchorRowKey()` の説明コメント・設計根拠が「直前の行は隙間の
  範囲外」と書いているが、これは**その隙間への最初のクリック**でのみ厳密に
  正しい。2回目以降のクリック（既に上から一部展開済みの隙間）では、
  `previousElementSibling` は実際には隙間の**内側**（既に展開済みの行）を
  指すことがある。それでも `shown.top`/`shown.bottom` は単調増加のみ
  （縮まない）なので、そのキーは再描画後も有効であり続け、結果としては
  正しい / 対応: 見送り（レビュー担当自身が「これ単体でのフォローアップ
  コミットには値しない」と評価。次にこのコードへ触る機会があればコメントを
  「隙間の外側」ではなく「（この時点までは）影響を受けていない」という、より
  正確な表現に直す）。

独立点検（別コンテキストの subagent）で must/should の指摘は無し。
`overflow-anchor: none` を基底の `.pane` へ一律に適用することが `.pane-left`/
`.pane-right`（ファイル一覧・コメント一覧、再構築を伴う巨大な展開操作を持たない
単純な一覧）にも具体的な悪影響を与えないこと、`anchorKey` によるフォールバックが
既存の「押したボタン自身がまだ存在する」経路（大半のクリック）を一切変更せず
無害に温存されること、`anchorKey` をクリック時点（DOM 破壊前）に計算しても、
↑/↓/すべて表示のいずれも `shown.top`/`shown.bottom` を単調増加させるだけなので
再描画後も有効であり続けること、`cssEscape` の使用が既存の `currentKey`/`again`
と同じ流儀で正しいこと、`file.expand.truncated` のヒント文（`<p class="hint">`）が
先頭に挿入されるケースで `previousElementSibling` が実際に `.row` でなくなる
