...
## D28: 展開ボタンクリック後に表示位置が動いてしまう不具合を修正した
  （前回の review が指摘し見送っていた nit が、実際の問題として顕在化）

- **背景**: ユーザーから「展開ボタンクリック後に表示位置が移動しないようにして
  ください」と報告。D27 の review で、押したボタン（↑/↓）が再描画後に消える
  （残り20行以下・隙間を閉じきった等）場合、`redrawFile()` のフォーカス復元が
  「ファイル内で最初に見つかった展開ボタン」等、無関係な離れた場所へフォール
  バックし得るという nit 指摘を受けていたが、そのときは「既存の仕組みで安全に
  動作しており壊れていない」として見送っていた。今回の報告で、これが実際に
  ユーザー体験として問題になっていたことが分かった。
  - 調査すると、**それとは別に、もう一つの・より支配的な原因**が見つかった:
    `redrawFile()` は展開のたびに `.file-body` を丸ごと `clear()` → 再構築する。
    大きなファイル（数十万px規模のスクロール高さ）でこれを行うと、ブラウザ既定の
    「スクロールアンカリング」（`overflow-anchor`、既定で有効）が、中身の大規模な
    入れ替えを検知して**アプリのコードとは無関係に scrollTop を自動で補正**して
    しまい、実測で500px超動くことを確認した。この上にアプリ自身の
    `focusInPlace()`（押したボタンをビューポート内に留めるための補正）がさらに
    重なるため、二重の補正がぶつかって位置が安定しなかった。
- **決定**: 2点を修正した。
  1. `.pane`（`.pane-left`/`.pane-center`/`.pane-right` 共通の基底クラス）に
     `overflow-anchor: none;` を追加し、ブラウザ既定の自動補正を止めた。位置の
     維持はアプリ側の `focusInPlace()` に一本化する。
  2. `redrawFile()` のフォーカス復元に、新設 `anchorRowKey(expanderRow)` による
     次善のフォールバックを追加した。押したボタン自身が消えている場合、その
     隙間の**外側**（直前・直後）にある行の `data-key` を掴んでおき（クリック時点、
     DOM が壊される前に取得）、再描画後にそれを引いて指し直す。「ファイル内で
     最初に見つかった展開ボタン」まで落ちる前に、より近い・安定した目印を試す。
- **理由・代替案**: `overflow-anchor: none` を追加せず `focusInPlace()` 側だけで
  対処する案も検討したが、ブラウザの自動補正はアプリのコードの**外側**で起きる
  ため、こちらだけをいくら直してもブラウザ側の補正と二重に効き続け、根本的な
  解決にならない（実測で確認済み）。`anchorRowKey` の目印は「隙間の直前の行」を
  最優先にした——展開方向に関わらず、隙間より前の内容は今回の展開で位置が
  変わらない（隙間の中身がどう増えても、隙間の外側には影響しない）ため、
  最も安定した基準点になる。「↓」方向のように押したボタン自身の絶対位置が
  下にずれる場合、`focusInPlace()` がビューポート内に留める程度の追従スクロールを
  行うことはあるが、これは新しく見えた行数に比例した・意図した最小限の動きで
  あり、無関係な場所へ飛ぶ既存の不具合とは別物（実測: 20行分の追従は約267px、
  1行あたり約13pxで行の高さと一致し、妥当な追従と判断した）。
- **検証**: playwright-core で、大きな隙間（403行）を「↑」で21回クリックし切って
  完全に閉じるまで、隙間の外側にある固定の行のビューポート内位置が**毎回0.0px**
  （完全一致）で変わらないことを実測した（修正前は2回目のクリックで424px、
  さらに後続のクリックでも同程度の乱れが起きていたことも同じ手法で先に実証
  した上での比較）。「↓」「すべて表示」方向、および split 表示でも、押した
  ボタン（またはその近傍）が展開後もビューポート内に留まる（無関係な遠い場所へ
  飛ばない）ことを確認した。既存の D27 のテスト（38箇所の展開帯の走査）・
  D25/D26 のテストも再実行し green のままであることを確認した。
  `python3 -m unittest`（140件）も green のまま。
