...
> **ラウンド17・最終**（ユーザーから「ソース差分の展開ボタンクリック後も差分量表示の
> 行が残ったままになっている」との報告を受け、広範な実機再現を試みたが再現できず、
> 続けてユーザーから「GitHubのように帯を1本にまとめれば根本的に発生しなくなる」との
> 提案を受けて対応した後の検証。decisions.md D25）。ラウンド1〜16の内容は本ファイル
> 末尾に残す。
...
- **D25 の元の再現条件は特定できていない**: ユーザーが報告した「展開ボタンを
  クリックしても『…N行』の帯が残る」という現象そのものは、playwright-core での
  網羅的な試行（実際の生成物37箇所の隙間・単独/混在方向のクリック・ファイル折りたたみ・
  表示モード切り替え等）でも一度も再現できなかった。対応は「原因を直す」のではなく
  「その種の不整合が構造的に起こり得ない形へ作り替える」（展開コントロールと
  ハンク見出しを1本の帯に統合）方向で行った——**元の症状の直接的な再現・原因確認は
  できていない**。ユーザーの環境・ブラウザ・操作手順が今回把握できた範囲を超えて
  いた可能性があり、今回の構造変更で解消しない場合は別の原因が残っている。
