...
- **playwright-core による実ブラウザでの操作確認**:
  1. **修正前の再現（先に実証）**: 大きな隙間（403行）を「↑」で連続クリックすると、
     2回目のクリックで424pxもビューポート位置がずれることを実測。ブラウザ標準の
     `scrollIntoView` 呼び出しを直接フックし、アプリのコードとは無関係な
     ブラウザ自身のスクロール補正（500px超）が先に起きていたことも実測で確認した
  2. **修正後**: `overflow-anchor: none` を追加した状態で同じ隙間を「↑」で
     21回連続クリックし、完全に閉じ切るまで**毎回0.0px**（固定した目印の
     ビューポート内位置が完全一致）であることを確認（計21回の delta 測定、
     すべて0.0px）。隙間を閉じきる境界・「すべて表示」への切り替わりを
     含めての一貫した安定性を確認
  3. 「↓」「すべて表示」方向、および split 表示でも、押したボタン（またはその
     近傍）が展開後もビューポート内に留まる（無関係な遠い場所へ飛ばない）ことを
     確認（4パターン×各1アサーション）
  4. D27（38箇所の展開帯の走査）のテストを再実行し、引き続き green であることを
     確認（今回の変更による回帰が無いこと）

- `python3 -m unittest discover -s docs/ClaudeCode/skills/other/diff-review-html/tests -p "test_*.py"`
  — 140 passed / 0 failed / 0 skipped（Python 側は無変更）
- `aidev smoke`（`.aidev/config.yml` の `smokeCommands` 3本）— pass (exit 0)
- **playwright-core による実ブラウザでの操作確認（実際の生成物にある全38箇所の
  展開帯を走査、計100アサーション、すべて pass）**:
  1. 残り20行以下（17箇所）は、いずれも「↑」「↓」ボタンが1つも無く
     「すべて表示」のみであることを確認
  2. 残り21行以上（21箇所）は、いずれも引き続き「↑」「↓」の少なくとも一方が
     あることを確認（既存のハンク境界による出し分けは維持されていること）
  3. すべての帯で「すべて表示」自体は常に存在すること
  4. 残り23行の隙間を実際に1回クリックして残り3行まで縮め、その場で「↑」
     「↓」が消えて「すべて表示」だけになる遷移をその場で実測
  - ライトテーマのスクリーンショットで、小さい隙間（15行）は「すべて表示」のみ、
    大きい隙間（104行）は「↑20行」「すべて表示」が並ぶ、という見た目の違いを確認
...
**ラウンド20では、修正前の状態を意図的に実機で再現・実測してから直した**
（`python3 -m unittest` 140件・smoke 3件は green、coding への差し戻しは無かった）。
展開ボタンの2回目のクリックで424pxのずれが起きることを最初に実証し（このずれの
存在自体が「失敗の証跡」に当たる）、ブラウザの `scrollIntoView` をフックして原因が
アプリのコードではなくブラウザ標準のスクロールアンカリングにあることを特定した
上で、`overflow-anchor: none` により解消したことを同じ手法で再実測して確認した。
