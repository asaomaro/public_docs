...
2. **取り込み口は 3 つ**——手動 / 埋め込み / ホストの `postMessage`（D9・D10・D11）。
   持たないのは「**ビューアが自分から取りに行く口**」で、
   `<script src>`（外部ファイル）とグローバル変数（暗黙の読み取り）を落とした。
   前者は「どのバンドルを読むのか」がファイル名任せになり、そのファイルを実行することにもなる。
   ホストは `view` の出力にある**空の `#bundle-data`** に書き込んで渡し
   （VSCode の webview の標準的な作り方そのもの）、
   開いたあとの差し替えは `postMessage`（再読み込みしないので位置が残る）。
