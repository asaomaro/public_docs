...
2. **生成物は外部ファイルを一切読まない**（D9）。当初は「`--load` を明示したときだけ焼き込む」形だったが、
   **ビューアがファイル名を指すと `.dreview` が複数あるときに表示が決まらない**という指摘を受けて撤去した。
   決め手は危険ではなく**決定性**。結果として、実行を伴う取り込み口も 1 つも無くなった。
   中身は「ファイルを開く」で人が選ぶか、ホストが渡す。
