...
2. **`<script src>` で読むのは「実行する」こと**（D3）。危ないのは自動読み込みがあること自体ではなく、
   **他人がくれたファイルをビューアが勝手に実行すること**。だから焼き込みは
   `--load` を明示したときだけにし、「決まった名前のファイルを隣に置けば読む」形には**しなかった**。
   他人から受け取ったバンドルは「ファイルを開く」（実行なし）で読む。
