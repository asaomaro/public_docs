...
| # | 口 | 誰が使う | 実行 |
|---|---|---|---|
| ① | `window.__DIFF_REVIEW_BUNDLE__` が既に定義済み | **ホストが自分で書いたときだけ** | しない（生成物はこの変数を埋めない） |
...
## 3. `<script src>` は**使わない**（D3 → **D9 で取りやめ**）

> 当初は「`--load` を明示したときだけ焼き込む」設計だった（D3）。
> **deliver 後に取りやめた**（D9）。理由は危険ではなく**決定性**——
> ビューアが特定のファイル名を指すと、`.dreview` が複数あるときに
> 「このビューアはどれを見ているのか」が決まらない。
> 配布物が、隣に何が置かれているかで表示を変えるのは、この skill の性質と相容れない。

いまの形:

| 生成コマンド | できるもの | 外部ファイルを読むか |
|---|---|---|
| `view --out viewer.html` | どのバンドルも自動で読まないビューア | **読まない** |
| `view --readonly --out viewer.html` | 同上・参照専用 | **読まない** |
| `html …` | 全部入り（中身は埋め込み済み） | **読まない** |

**生成物に `<script src=` は一切出ない**（テストで固定）。
中身は「ファイルを開く」で人が選ぶか、ホストが渡す。副次的に、
**実行を伴う取り込み口が 1 つも無くなった**（research R1 は解消）。
...
- AC5: ホストからの経路は 2 本——`postMessage`（口③）と `window.__DIFF_REVIEW_BUNDLE__` の注入（口①）。
  **どちらも生成物がファイルを読みに行かない**。口①はホストが流し込む HTML に自分で書く形なので、
  起動時から中身が出る（案内が一瞬見えない）。
...
- AC15: **実行を伴う取り込み口を用意しない**（D9）ので、この AC は「用意する場合は…」の
  前提ごと外れる。代わりに「生成物に `<script src=` が出ない」ことをテストで固定する。
