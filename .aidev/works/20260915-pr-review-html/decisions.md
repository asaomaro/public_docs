...
## D16: deliver 後に見つけた must を、PR マージ前に同じブランチで直す

- **背景**: PR #21 を出したあと、サンプル HTML を生成する過程で「差分がテンプレートの
  プレースホルダ文字列を含むと埋め込み JSON が壊れる」欠陥を踏んだ（この skill 自身の差分で必ず再現）。
- **決定**: 新しい work は起こさず、**同じブランチに追加コミット**して PR #21 を更新する。
  記録は `review.md` の「deliver 後の発見」節（人間の PR 指摘とは母集団を分ける）。
- **理由・代替案**: PR が未マージなので、着地点は同じ。別 work にすると「未マージの PR に
  既知の must が残ったまま」の状態を作る。`aidev` の工程はすべて承認済みなので、
  工程をやり直すのではなく**着地の修正**として扱う。
- **影響**: `files_changed` / `insertions` の刻印（deliver の metrics）は更新しない——
  1 work の規模は deliver 時点の値を正典とし、後追い修正で書き換えると work 間の比較が壊れる。
  追加分は PR の差分として見える。
