...
## deliver 後の発見（2026-09-16T03:19:51Z・PR #21 マージ前）

サンプル HTML を作って見せる過程で踏んだ欠陥。人間のレビュー指摘ではないので
「PR レビュー（人間）」節ではなく、この節に分けて記録する（母集団を混ぜない）。

- [must][conv:-] `diff_review.py` の `render_html()` が**プレースホルダを順に `str.replace` していた**ため、
  **差分の中にテンプレートのプレースホルダ文字列が含まれていると埋め込み JSON が壊れる**。
  先に差し込んだ差分 JSON の中を、次の `__REVIEW_DATA__` の置換が書き換えてしまう。
  **この skill 自身の差分をレビューすると必ず踏む**（`templates/page.html` の追加行が
  `__DIFF_DATA__` / `__REVIEW_DATA__` をそのまま含むため）。
  実測: 画面が真っ白になり `Cannot read properties of null (reading 'target')` が 2 件。
  / 対応: 修正済（1 回の走査で全プレースホルダを置換する `PLACEHOLDER_RE.sub` に変更）＋
  退行検知テストを追加（旧実装に戻すとこのテストだけが落ちることを確認）

**この欠陥を工程で捕まえられなかった理由**（retro の材料）:
test の受け入れ確認も unittest も、**自分自身を入力にする**ケースを持っていなかった。
差分に「テンプレートの語彙」が現れるのは自己適用のときだけで、合成した小さなリポジトリでは出ない。
